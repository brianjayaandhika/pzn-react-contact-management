...
import { Task, TaskStatus } from "@/lib/api/task/task.types";
...
function Badge({
  ...
  return (
    <div
      className={`${customCn} py-1 px-3 bg-gray-700 border border-gray-600 rounded-2xl text-sm font-bold`}
    >
...
function PriorityCn(priority: 1 | 2 | 3) {
  let el;
...
export default function TaskCard({ task }: { task: Task }) {
  ...
  return (
    <div className="w-full h-full group relative flex flex-col justify-between rounded-2xl border border-slate-700/70 bg-slate-800 shadow-[0_10px_30px_-10px_rgba(0,0,0,.6)] hover:shadow-[0_18px_15px_-12px_rgba(255,255,255,.1)] hover:bg-slate-700 transition-all duration-300 overflow-clip cursor-pointer">
      <div className="p-4 flex flex-col gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Badge
            label={`${task.priorityLabel} Priority`}
      ...
      <p className="p-4 self-end text-sm text-white/50">
