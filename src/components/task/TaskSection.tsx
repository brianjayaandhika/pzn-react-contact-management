...
export default function TaskSection({
  ...
  function selectTaskToRender(): Task[] {
    ...
    if (tasks) {
      const groupByPriority = _.groupBy(tasks, (e) => e.priorityLabel);

      console.log("groupByPriority", groupByPriority);
  ...
  return (
    <div className="w-full min-h-full px-32 flex flex-col gap-6 relative mt-20 scroll-smooth">
      ...
      <span className="flex gap-2 justify-center items-center align-middle cursor-pointer text-sm text-white/80 hover:text-white/50">
        {isHighPriority && renderTask.length > 2
          ? "You have other urgent tasks! View Other Task"
          : "View Other Task"}
